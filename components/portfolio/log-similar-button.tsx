'use client'
import { useQuickAdd } from '@/app/(dashboard)/providers'

type EntryType = 'case' | 'teaching' | 'reflection' | 'procedure'
const QUICK_ADD_TYPES: EntryType[] = ['teaching', 'reflection', 'procedure']

export default function LogSimilarButton({ category, tags }: { category: string; tags?: string[] }) {
  const { openQuickAdd } = useQuickAdd()
  if (!QUICK_ADD_TYPES.includes(category as EntryType)) return null
  return (
    <button
      onClick={() => openQuickAdd({ type: category as EntryType, tags: tags ?? [] })}
      className="flex items-center gap-2 px-3.5 py-2 text-sm font-medium text-[var(--text-secondary)] border border-white/[0.08] rounded-lg hover:text-[var(--text-primary)] hover:border-white/[0.15] transition-colors"
    >
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/>
      </svg>
      Log similar
    </button>
  )
}
