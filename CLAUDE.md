...
- **Specialty tagger (Batch 3 partial / Batch 4): Batch 3 ✅ standardised `specialty_tags` → "Linked specialties"** on the case detail, quick-add, the landing mock and the `specialty-tag-select` doc comment, and **dropped the case-form competency-theme picker** (F-017: cases no longer write `interview_themes`; the column + all portfolio usage stay). **STILL PENDING (Batch 4 / F-046): retiring `interview_ready_for` ("Marked ready for") app-wide** — it remains live on portfolio entries (form control, card badge, detail, filter, export targeter, public API). Scoring engine (`specialty_applications`/`specialty_entry_links`) stays separate and unchanged.
