...
- **Brand blue is now single-source via an RGB-channel token (2026-06-30, `48dc628`).** `--accent-rgb: 27 111 217` in `:root` (globals.css) is the ONE place the brand colour lives; `--accent`/`--blue`/`--focus-ring` derive from it (`rgb(var(--accent-rgb))`) in both themes. The Tailwind `accent` token DEFAULT is `rgb(var(--accent-rgb) / <alpha-value>)`, so `bg-accent`, `bg-accent/15`, `border-accent/40`, `text-accent/80` all work and all trace to that one line — **rebrand off blue = change `--accent-rgb`.** The prior codemod swept all 153 `*-[#1B6FD9]` arbitrary values to `*-accent` across 50 files (zero visual change; verified live both themes). **NEW brand-blue accents/tints MUST use `bg-accent/NN` etc., NOT a hardcoded `bg-[#1B6FD9]/NN`.** Bare inline `#1B6FD9` remains correct only where vars can't reach: SVG logos, email-template HTML, the PDF renderer, and standalone `global-error`/`/r/[code]` inline styles.
