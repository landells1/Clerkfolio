...
- **Tailwind tokens (`tailwind.config.ts`) now resolve to these vars** (`surface-*`, `ink[-soft/-dim]`, `fg-{1..4}`, `accent`, `border-{subtle,default,strong}`, `pill-*`, `boxShadow card/elevated/modal`). So `bg-surface-2`, `text-ink`, `border-default`, etc. flip automatically. NOTE: these are plain `var()` colours → **Tailwind opacity modifiers (`/50`) don't work on them**; use a dedicated token or `bg-[var(--…)]`.
