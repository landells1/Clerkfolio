...
**Finalisation reconciles `file_size` with the REAL object size (2026-07-11):** `/api/upload/authorize` necessarily trusts the client-declared `fileSize` (the bytes don't exist yet), so both finalisation paths now write `file_size = blob.size` and re-enforce the 50 MB cap + storage quota against it (`lib/upload/quota.ts` `checkFinalisedUploadSize`), rejecting with object-then-row removal when busted — closes the declared-1-byte quota bypass; honest clients see no change. Implemented in `/api/upload/verify` AND the `scan-evidence` edge function, whose source is now TRACKED at `supabase/functions/scan-evidence/` (fetched verbatim from the deployed v4 then patched — **deploy owed**; until deployed, the primary edge path doesn't size-check, only the verify fallback does). Rows finalised before this fix keep their declared sizes (optional one-time backfill = owner decision).
