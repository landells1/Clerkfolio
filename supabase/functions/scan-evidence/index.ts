...
//
// Source of truth note: this file mirrors the DEPLOYED scan-evidence function
// (retrieved from the live project before the size-reconciliation change was
// added). Deploying is a separate, owner-gated step; keep this file and the
// deployed version in sync.

import { createClient } from 'https://esm.sh/@supabase/supabase-js@2'

const BUCKET = 'evidence'
const CLAMAV_CHUNK_BYTES = 64 * 1024
// Per-file cap. Must match MAX_FILE_BYTES in lib/supabase/storage.ts and the
// evidence bucket's 50 MB file cap.
...
// Base-ten storage formatting, mirroring formatStorageQuota in
// lib/entitlements/limits.ts (1 GB = 1000 MB).
function formatStorageQuota(mb: number) {
  if (mb >= 1000) {
    const gb = mb / 1000
...
Deno.serve(async (req) => {
  ...
  // Reconcile the stored size with the REAL object size, then re-check the
  // caps. /api/upload/authorize trusts the client-declared fileSize (the bytes
  // are not uploaded yet), so a crafted client can declare 1 byte, PUT ~50 MB,
  // and make the quota meter read near-zero (get_profile_entitlements sums
  // file_size). Finalisation is the first point the true size is known.
  // Mirrors /api/upload/verify (the fallback path) and checkFinalisedUploadSize
  // in lib/upload/quota.ts.
  if (!downloadError && blob) {
    ...
    if (sizeUpdateError) {
      // Without the corrected size persisted, the quota check would run
      // against the client-declared figure. Leave the row unfinalised so the
      // orphan cron reclaims it (row + object) if retries never succeed.
    ...
    if (actualSize <= MAX_FILE_BYTES) {
      const { data: entitlements, error: entitlementsError } = await service
        .rpc('get_profile_entitlements', { p_user_id: user.id })
        .single()
      if (!entitlementsError && entitlements) {
        const row = entitlements as { storage_quota_mb: number | null; storage_used_mb: number | null }
        // Base-ten units (1 MB = 1,000,000 bytes), matching /api/upload/authorize
        // and storage_used_mb. usedBytes already includes this file at its
        // corrected size. Fail-open on RPC drift, matching the app routes'
        // fetchSubscriptionInfo fallback - the size reconciliation above is the
        // load-bearing part of the fix.
        quotaMB = row.storage_quota_mb ?? 100
        const quotaBytes = quotaMB * 1_000_000
        const usedBytes = (row.storage_used_mb ?? 0) * 1_000_000
        overQuota = usedBytes > quotaBytes
      }
    }

    if (actualSize > MAX_FILE_BYTES || overQuota) {
      // Reject this NEW upload: remove the storage object and its row (link
      // rows cascade via FK ON DELETE CASCADE). Previously stored files are
      // untouched, so the "over-quota never deletes existing data" rule
      // (F-040) holds. Only delete the row once the object is gone: the
      // orphan cron finds objects via their rows, so a row deleted ahead of a
      // failed storage remove would leak the object forever.
      const { error: removeError } = await service.storage.from(BUCKET).remove([file.file_path])
      if (!removeError) {
        await service.from('evidence_files').delete().eq('id', fileId)
      }
      const message = actualSize > MAX_FILE_BYTES
        ? 'File too large. Maximum size is 50 MB.'
        : `Storage limit reached (${formatStorageQuota(quotaMB)}). Your existing files are safe — delete some or upgrade to free up space.`
