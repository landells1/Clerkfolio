            ...
            <article key={revision.id} className="rounded-2xl border border-white/[0.08] bg-[#141416] p-5">
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  {revisions[index + 1] && (
                    <Link href={`/portfolio/${id}/history/diff?a=${revisions[index + 1].id}&b=${revision.id}`} className="rounded-lg border border-white/[0.08] px-3 py-1.5 text-xs font-medium text-[rgba(245,245,242,0.72)]">
                      Diff
                    </Link>
                  )}
                  <RestoreVersionButton revisionId={revision.id} entryType="portfolio" entryPath={`/portfolio/${id}`} />
                </div>
              </div>
