      ...
      <main className="min-w-0 flex-1 p-4 sm:p-5">
        ...
        <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
          ...
          <div className="min-w-[180px]">
            ...
            <div className="h-1.5 rounded-full bg-white/[0.08]"><div className="h-full w-[62%] rounded-full bg-accent" /></div>
          </div>
        </div>
        <div className="overflow-hidden rounded-2xl border border-white/[0.06] bg-[var(--bg-surface)]">
          {rows.map(([domain, box, evidence, done, priority]) => (
            <div key={domain} className="grid grid-cols-[20px_1fr_auto] items-center gap-3 border-b border-white/[0.04] px-3 py-2.5 last:border-b-0">
              <span className={`flex h-4 w-4 items-center justify-center rounded border text-[10px] ${done ? 'border-accent bg-accent text-surface' : 'border-white/[0.18] text-ink-dim'}`}>{box}</span>
