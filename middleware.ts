...
export const config = {
  matcher: [
    ...
    //  - Common static image extensions, and plain-text crawler files
    //    (llms.txt, the IndexNow key file, security.txt): crawlers and the
    //    IndexNow validator must get the raw 200 text response.
    '/((?!_next/static|_next/image|favicon.ico|sw\\.js|manifest\\.webmanifest|manifest\\.json|robots\\.txt|sitemap\\.xml|.*\\.(?:svg|png|jpg|jpeg|gif|webp|txt)$).*)',
  ],
}
